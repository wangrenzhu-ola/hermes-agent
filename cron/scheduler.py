...
def run_job(
    ...
    try:
        ...
        # A cron job's explicit reasoning_effort is the narrowest override and
        # therefore wins over both per-model and profile defaults. Preserve the
        # raw YAML value so ``False`` continues to mean thinking disabled.
        if job.get("reasoning_effort", None) is not None:
