...
class TestCronPerModelReasoningConfig:
    ...
    @pytest.mark.parametrize(
        ("job_effort", "expected"),
        [("low", {"enabled": True, "effort": "low"}), (False, {"enabled": False})],
    )
    def test_explicit_job_effort_wins_over_model_and_global_defaults(
        self, job_effort, expected
    ):
        """Cron's job-level override remains the narrowest reasoning scope."""
        ...
        assert parse_reasoning_effort(job_effort) == expected
