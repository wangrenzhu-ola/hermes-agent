...
class TestCredentialPoolSeedsFromDotEnv:
    ...
    def test_dotenv_wins_over_os_environ(self, isolated_hermes_home, monkeypatch):
        """_seed_from_env prefers .env over os.environ — .env is the user's
        deliberate config while os.environ may carry stale inherited values
        from parent shells (regression fix #18254)."""
        _write_env_file(isolated_hermes_home, DEEPSEEK_API_KEY="sk-dotenv-fresh")
        monkeypatch.setenv("DEEPSEEK_API_KEY", "sk-env-stale-from-shell")
